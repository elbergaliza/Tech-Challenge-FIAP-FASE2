...
def atualizar_dash(_):
    ...
    if historico["hiperparametros"]:
        ...
        fig_dist = px.box(df_hp, template="plotly_dark", title="Distribuição dos Hiperparâmetros")

        corr = df_hp.corr(numeric_only=True)
        fig_corr = px.imshow(corr, text_auto=True, template="plotly_dark", title="Correlação entre Hiperparâmetros")
    else:
        fig_dist = go.Figure()
        fig_corr = go.Figure()
